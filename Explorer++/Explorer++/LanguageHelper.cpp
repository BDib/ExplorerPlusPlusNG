	...
	const CommandLine::Settings *commandLineSettings, const Config *config)
{
	...
	languageDllPath.replace_filename(L"translations");
	languageDllPath.append(std::format(L"Explorer++{}.dll", *languageCode));

	if (!std::filesystem::exists(languageDllPath))
	{
		languageDllPath = currentProcessPath;
